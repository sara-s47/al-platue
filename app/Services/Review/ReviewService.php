...
class ReviewService
{
    public function __construct(
        ...
        protected ReviewDimensionRepositoryInterface $reviewDimensionRepository,
    ...
    {
        return $this->reviewRepository->paginateAdmin($perPage, $filters);
    }

    public function listForUser(int $userId, int $perPage = 15): LengthAwarePaginator
    {
        return $this->reviewRepository->listForUser($userId, $perPage);
    }

    public function listDimensions(int $perPage = 15): LengthAwarePaginator
    {
        return $this->reviewDimensionRepository->paginate($perPage);
    }

    public function createDimension(array $data): Model
    {
        return $this->reviewDimensionRepository->create([
            'name' => $data['name'],
    ...
    public function moderate(int $reviewId, string|ReviewStatus $status): Model
    {
        $status = $status instanceof ReviewStatus ? $status : ReviewStatus::from($status);

        if (! in_array($status, [ReviewStatus::Approved, ReviewStatus::Rejected, ReviewStatus::Hidden, ReviewStatus::Pending], true)) {
